...
class InputTracNghiem_TN_COBAN implements ComponentInput, ObserverInput {
    ...
    InputTracNghiem_TN_COBAN(GameEngine ger) {
    ...
    public void handleInput(MotionEvent event, GameState gameState, GameEngine ge) {
        ...
        myRect Exit = (myRect)controls.get(SpecTracNghiem_TN_COBAN.EXIT);
        myTracNghiem tracNghiem = (myTracNghiem)controls.get(SpecTracNghiem_TN_COBAN.TN);
        ...
        if(gameState.getKey()) {
            if (eventType == MotionEvent.ACTION_UP || eventType == MotionEvent.ACTION_POINTER_UP) {
                if (Exit.rect.contains(x, y)) {
                    if (gameState.getgd() == gameState.GD_TRACNGHIEMC_COBAN && !gameState.getDiaglog7()&& !gameState.getDiaglog1() && !gameState.getDiaglog2()&& !gameState.getDiaglog3()) {
