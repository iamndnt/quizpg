...
class InputTracNghiem_TN_TRUNGCAP implements ComponentInput, ObserverInput {
    ...
    InputTracNghiem_TN_TRUNGCAP(GameEngine ger) {
    ...
    public void handleInput(MotionEvent event, GameState gameState, GameEngine ge) {
        ...
        myRect Exit = (myRect)controls.get(SpecTracNghiem_TN_TRUNGCAP.EXIT);
        myTracNghiem tracNghiem = (myTracNghiem)controls.get(SpecTracNghiem_TN_TRUNGCAP.TN);
        ...
        if(gameState.getKey()) {
            if (eventType == MotionEvent.ACTION_UP || eventType == MotionEvent.ACTION_POINTER_UP) {
                if (Exit.rect.contains(x, y)) {
                    if (gameState.getgd() == gameState.GD_TRACNGHIEM_TRUNGCAP) {
                        gameState.setgd(gameState.GD_TRACNGHIEM_DASHBOARD);
                        gameState.clearKey();
                    }
                }else if(Option1.contains(x,y)){
                    if (gameState.getgd() == gameState.GD_TRACNGHIEM_TRUNGCAP && !gameState.getDiaglog7()&& !gameState.getDiaglog1() && !gameState.getDiaglog2()&& !gameState.getDiaglog3()) {
                        ge.setANS("A");
